...
void setup(){
  ...
  while (i < numberOfFrames) {
    gif [i] = loadImage ("stellar"+i+".gif"); //multiple copies
    i=i+1;
  }
...
void draw(){
  ...
  if(mode==INTRO) {
    textFont(createFont("ArimaMadurai-SemiBold", 45));
    drawIntro();
  }
  if(mode==GAME)  {
    drawGame();
...
void drawGame(){
  ...
  text("Teleport (Z) & Pause (P)",barLeft,barY+14);
...
//2 conditions (Run out of lives/asteroids)
if(health<=0){
  gameoverLose();
}
...
void drawGameOver(){
  ...
  //text for gameover 2 modes
  String endText;
  if(gameoverState==1){
    endText="CONGRATULATIONS! YOU WIN!";
}
  else{
    endText="AWWW! YOU LOSE!";
}


  text(endText,width/2,height/2-50);
