...
//FIXED 1. Function Call 2. While loops
...
void drawStructures() {
  int count = 0;
  while (count < 4) {
    float x = 50 + count * 250;
    igloo(x, 50);
    structureComponent(x);
    spots(x, 350);
    door(x, 350);
    roof(x);
    doorpart(x);
...
void drawButterflies() {
  ...
  while (i < 5) {
    float x = 50 + i * 200;
    butterfly(random(x, x + 100), random(35, 200));
    i++;
  }
}
...
void igloo(float x, float y) { //How can igloo randomly position without changing inside?
  pushMatrix();
  translate(x, y);
  popMatrix();
}
...
void drawFlowers() {
  ...
  while (i < 6) {
    float x = 50 + i * 160;
    flower(random(x, x + 100), random(500, 700));
    i++;
  }
}
