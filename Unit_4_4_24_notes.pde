...
void setup() {
  ...
  //int count=0;
  //while(count<1000){
  // face(random(0,500),random(0,500),random(0.25,1)); //infinite loops (don't want)
  //  count=count+1;
  //}
...
void draw() { //on loop
  background (beige);
  face(random(0,255),random(0,255),random(0,255));
  fill(random(0,255),random(0,255),random(0,255));//r,g,b
...
//the variables only exist within the function eg. draw/setup 
  ...
  hair();
  float r,g,b;
  //now the eye colors are all the same!
  r=random(0,255,r,g,b);//range is 0 to 255
  g=random(0,255,r,g,b); 
  b=random(0,255,r,g,b);
...
void eye(float x, float y, float r, float g, float b) {
  fill(random(0,255),random(0,255),random(0,255)); //random colors (hue, saturation, brightness)
  
  //set the set of colors into local variables (exist over global variables)
